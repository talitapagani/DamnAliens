Crafty.scene("main", function() {
	...
	require(elements, function() {
		...
		function setLevel(_level, _numNaves, _valorPonto, _naveSpeed) {
			...
			infc['level_tween'] = new CustomText({'x': 0, 'y': -48, 'w': 800, 'h': 48, 'text': 'Nível '+nivel, 'fontSize': '48', textAlign: 'center'}); 
			infc['level_tween'].getEntity().addComponent("Tweener");
			infc['level_tween'].getEntity().addTween({y: 250}, 'easeOutBounce', 80, function() {
				this.addTween({alpha: 0}, 'linear', 100, function() {
					infc['level_tween'].remove();
				});
			});
